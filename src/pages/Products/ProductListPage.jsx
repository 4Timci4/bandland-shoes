...
import { FaUndo, FaTh } from 'react-icons/fa';
...
function ProductListPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true);
        const [fetchedProducts, fetchedColors, fetchedCategories] =
          await Promise.all([
    ...
    if (selectedColors.length > 0) {
      result = result.filter((group) =>
        group.variants.some((variant) =>
          selectedColors.includes(variant.color_id)
        )
      );
    }
            ...
            <ul className="color-filter-list">
              {colors.map((color) => (
                <li
                  ...
                  className={selectedColors.includes(color.id) ? 'active' : ''}
                >
                  <span
                ...
                >
                  {paginatedProducts.map((groupedProduct) => {
                    ...
                    const subCategory = categories
                      ...
                      .find((sc) => sc.id === firstVariant.sub_category_id);
                    const category = subCategory
                      ? categories.find((c) => c.id === subCategory.category_id)
                      : null;
                    return (
                      <ProductCard
                        key={groupedProduct.base_product_id}
