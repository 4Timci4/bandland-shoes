...
const ManageProducts = () => {
  ...
  const filteredProducts = useMemo(() => {
    return products
      .filter(product => {
        ...
        const subCategoriesOfCat = categories.find(c => c.id === parseInt(catId, 10))?.sub_categories.map(sc => sc.id);
      ...
      .filter(product => {
        ...
        const baseSkuMatch = product.base_sku ? product.base_sku.toLowerCase().includes(term) : false;
        return (
  ...
  const columns = [
    ...
    { key: 'base_sku', header: 'Ana SKU', render: item => item.base_sku || 'N/A', className: 'col-base-sku' },
    { key: 'sku', header: 'Varyant SKU', className: 'col-sku' },
    { key: 'price', header: 'Fiyat', render: item => `${item.price} ₺`, className: 'col-price' },
    { key: 'stock_quantity', header: 'Stok', className: 'col-stock' },
    {
      key: 'category',
      header: 'Kategori',
      render: (item) => {
          const subCategory = categories.flatMap(c => c.sub_categories).find(sc => sc.id === item.sub_category_id);
  ...
  const formFields = [
    { name: 'base_sku', label: 'Ana Ürün SKU', required: true, description: "Bu SKU'ya sahip bir ana ürün sistemde mevcut olmalı." },
    ...
    { name: 'price', label: 'Fiyat (₺)', type: 'number', required: true, step: "0.01" },
    { name: 'stock_quantity', label: 'Stok Adedi', type: 'number', required: true },
    { name: 'image_url', label: 'Görsel', type: 'file' },
    { name: 'sub_category_id', label: 'Kategori', type: 'select', options: getSubCategoryOptions(), required: true },
    { name: 'color_id', label: 'Renk', type: 'select', options: colors.map(c => ({ value: c.id, label: c.name })), required: true }
  ];
